...
volumes = [get_audio_clip_avg_volume(get_clip_audio_for_second(i)) for i in range(0, int(clip_duration_multiplied - 1))]
...
for i in range(0, int(clip_duration_multiplied) - 1):
    ...
    if is_speaking_in_clip(volumes[i]):
        ...
        current_subclip_starting_point = i + SILENCE_BUFFER
        current_subclip_ending_point = i + SILENCE_BUFFER

final_video_clip = concatenate(final_video_subclips)
final_video_clip.to_videofile('cut_video3.mp4')
